#!/usr/bin/env python3
"""
Assert Family ID Validation Script

This script validates that:
1. All font directories contain a METADATA.pb file
2. The family name from METADATA.pb matches the folder name (family id)
   where family id is no-spaces, all lowercase version of the family name

Usage:
    python assert_family_id.py [--fonts-dir /path/to/fonts] [--verbose]
"""
...
def normalize_family_name(family_name: str) -> str:
    """
    Convert family name to family id format:
    - Remove all spaces
...
def parse_metadata_pb(metadata_path: str) -> Dict:
    """Parse METADATA.pb file using gftools protobuf and extract family information."""
    try:
        ...
        # Extract family name
        family_name = metadata.name if metadata.HasField('name') else None
        ...
        return {'family_name': None, 'metadata': None}


def validate_family_id(font_dir: str, verbose: bool = False) -> List[Tuple[str, str]]:
    """
    Validate family id in a single font directory.
    ...
    folder_name = os.path.basename(font_dir)
    ...
    if not metadata.HasField('name'):
        issues.append(('ERROR', f'METADATA.pb missing family name'))
        return issues

    family_name = metadata.name
    expected_folder_name = normalize_family_name(family_name)

    if verbose:
        print(f"  Family name from METADATA.pb: '{family_name}'")
        print(f"  Expected folder name: '{expected_folder_name}'")
        print(f"  Actual folder name: '{folder_name}'")

    # Validate that folder name matches expected family id
    if folder_name != expected_folder_name:
        issues.append(
            ('ERROR', f'Folder name "{folder_name}" does not match family id "{expected_folder_name}" (from family name "{family_name}")'))
...
def main(fonts_dir, verbose, output):
    ...
    for font_dir in sorted(font_directories):
        ...
        issues = validate_family_id(font_dir, verbose)
    ...
    print(f"  Skipped fonts: {skipped_fonts}")
    ...
    if all_issues:
        ...
        if output:
            ...
            print(f"\nIssues written to: {output}")

        sys.exit(1)
    else:
        print(f"\n✅ All fonts validated successfully!")
        print(f"   All {valid_fonts} fonts have valid family IDs")
        print(f"   All folder names match their family names from METADATA.pb")
